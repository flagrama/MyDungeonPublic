...
public class Player : MovingObject {
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.tag == "Exit"
            && (int)Mathf.Round(collision.gameObject.transform.position.x) == (int)Mathf.Round(transform.position.x)
            && (int)Mathf.Round(collision.gameObject.transform.position.y) == (int)Mathf.Round(transform.position.y))
        {
            enabled = false;
    ...
    public void RecoverHealth(int recover)
    {
        curHealth += recover;
        UpdateHealth();
        HudManager.instance.AddMessage(this.displayName + " recovered " + recover + " health");
    }
