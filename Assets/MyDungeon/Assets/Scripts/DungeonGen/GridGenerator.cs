...
public class GridGenerator : MonoBehaviour
{
    ...
    {
        Exit, Item, Floor, Wall, Creature, Player,
    }
    ...
    void SetupTilesArray()
    {
        ...
        GameManager.instance.board = new TileType[columns, rows];

        // Go through all the tile arrays...
        for (int i = 0; i < GameManager.instance.board.GetLength(0); i++)
        {
            for (int j = 0; j < GameManager.instance.board.GetLength(1); j++)
            {
                GameManager.instance.board[i, j] = TileType.Wall;
            }
        }
    ...
    void SetTilesValuesForRooms()
    {
        ...
        for (int i = 0; i < rooms.Length; i++)
        {
            ...
            for (int j = 0; j < currentRoom.roomWidth; j++)
            {
                ...
                for (int k = 0; k < currentRoom.roomHeight; k++)
                {
                    ...
                    GameManager.instance.board[xCoord, yCoord] = TileType.Floor;
                }
            }
            ...
            if (itemCount > 0)
            {
                ...
                if (r < 30)
                {
                    ...
                    GameManager.instance.board[itemPosX, itemPosY] = TileType.Item;
                    itemCount--;
                }
                else if (r >= 30 && r < 60)
                {
            ...
            if (creatureCount > 0)
            {
                ...
                if (r < 40)
                {
                    ...
                    GameManager.instance.board[creaturePosX, creaturePosY] = TileType.Creature;
                    creatureCount--;
                }
                else if (r >= 40 && r < 50)
                {
        ...
        GameManager.instance.board[exitX, exitY] = TileType.Exit;
        GameManager.instance.board[playerX, playerY] = TileType.Player;
    }
    ...
    void InstantiateTiles()
    {
        ...
        for (int i = 0; i < GameManager.instance.board.GetLength(0); i++)
        {
            for (int j = 0; j < GameManager.instance.board.GetLength(1); j++)
            {
                ...
                if (GameManager.instance.board[i, j] == TileType.Wall)
                {
                    // ... instantiate a wall over the top.
                    InstantiateFromArray(wallTiles, i, j);
                }

                if (GameManager.instance.board[i, j] == TileType.Player)
                {
                    Vector3 pos = new Vector3(i, j, 0);
                    Instantiate(player, pos, Quaternion.identity);
                }

                if (GameManager.instance.board[i, j] == TileType.Exit)
                {
                    Vector3 pos = new Vector3(i, j, 0);
                    Instantiate(exit, pos, Quaternion.identity);
                }

                if (GameManager.instance.board[i, j] == TileType.Creature)
                {
                    InstantiateFromArray(creatures, i, j);
                }

                if (GameManager.instance.board[i, j] == TileType.Item)
                {
